...
_writer_var: ContextVar[asyncio.StreamWriter] = ContextVar("_writer_var") # 最后的= ContextVar("_writer_var")真正创建了对象，而前面的只是一个类型注解，告诉人们我将要存什么东西进去。"_writer_var"是调试名，在调试器中显示的名字，是可选的，也可以写其他名字
...
class SocketServer:
    ...
    async def start(self) -> str:
        ...
        self._server = await asyncio.start_server( #调用 asyncio.start_server 注册一个 TCP 服务器，然后立刻返回。真正处理请求的逻辑在 _handle_connection 和 _read_loop 中，它们是作为后台任务独立运行的
            self._handle_connection, # 告诉事件循环：“当 host:port 有新连接时，帮我启动 _handle_connection 协程来处理它。” cli.commands.ping.py中执行reader, writer = await asyncio.open_connection(config.host, config.port)的时候，会自动执行回调函数_handle_connection
            host=self._host,
            port=self._port,
            limit=_MAX_LINE_BYTES, # 1imit=_MAX_LINE_BYTES 把单行帧（single-line frame）限制在64MB，防止一个异常客户端发出超大行把内存顶爆。
        )
    ...
    async def _read_loop(
        ...
        while True:
            ...
            # 每条命令独立作为 task 执行，避免长时间运行的 handler（如 session.send_message）阻塞读循环，使 permission.respond 等并发命令能被及时处理
    ...
    async def _handle_line(self, line: bytes, writer: asyncio.StreamWriter) -> None:
        ...
        if self._trace is not None:
            ...
            self._trace.emit( # 埋点1，服务端收到命令解析成功后
                TraceRecord(
                    ts=_now(),
        ...
        handler = self._handlers.get(req.method) #获取方法
        if handler is None:
            await self._send(
                writer,
        ...
        try:
            result = await handler(req.params) #根据客户端发送的命令调用对应的路由函数
        ...
        try:
            await self._send(writer, JsonRpcSuccess(id=req.id, result=result_data)) # 把 result 打包成 JSON-RPC 响应，写回客户端。
