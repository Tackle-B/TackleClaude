...
async def invoke_tool(
    ...
    *,
    ...
    tool = registry.get(tool_call.name) #从 ToolRegistry 中按名称查找工具
    if tool is None:
        return await _fail(
    ...
    if tool.params_model is not None:
        try:
            tool.params_model.model_validate(dict(tool_call.input)) # 参数校验
    ...
    if permission_manager is not None:
        ...
        allowed, decision = await permission_manager.check_and_wait( #通常返回True, 'allow once'
            tool_use_id=tool_call.id,
            tool_name=tool_call.name,
            params=dict(tool_call.input),
    ...
    for attempt in range(1, _MAX_RETRIES + 2):
        ...
        try:
            result = await asyncio.wait_for(
                tool.invoke(dict(tool_call.input)), timeout=timeout #调用工具的invoke方法，真正执行工具
            )
            ms = elapsed()
